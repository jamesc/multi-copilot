...
function Clear-CopilotWorktree {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        Clear-CopilotWorktree -All

    .EXAMPLE
        Clear-CopilotWorktree -DryRun
    #>
    ...
    # Confirm before removing (skip prompts during -WhatIf)
    ...
    $removed = 0
    foreach ($container in $containersToRemove) {
        if ($PSCmdlet.ShouldProcess($container.Name, "Stop and remove container")) {
    ...
    if ($projectVolumes.Count -gt 0) {
        ...
        $confirmVolumes = if ($NoConfirm -or $WhatIfPreference) { "y" } else { Read-Host "Remove unused volumes? (y/N)" }
        if ($confirmVolumes -eq 'y' -or $confirmVolumes -eq 'Y') {
            foreach ($volume in $projectVolumes) {
                if ($PSCmdlet.ShouldProcess($volume, "Remove volume")) {
